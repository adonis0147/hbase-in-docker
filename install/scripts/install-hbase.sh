...
cp /root/install/conf/hbase/* "${hbase_conf_path}/"

chown -R hadoop:hadoop "${HOME}"/hbase-*

sudo -u hadoop mkdir -p /data/conf
sudo -u hadoop mv "${hbase_conf_path}" /data/conf/hbase-conf
sudo -u hadoop cp /data/conf/hadoop-conf/hdfs-site.xml /data/conf/hbase-conf/
